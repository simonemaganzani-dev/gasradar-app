...
const OCM_API_KEY = "7aebe937-e593-4f32-b323-87ccab78c335";
...
export default function GasRadar() {
  ...
  useEffect(() => {
    ...
    fetch(`https://api.openchargemap.io/v3/poi/?latitude=${location.lat}&longitude=${location.lon}&distance=${radius}&distanceunit=KM&maxresults=20&compact=false&verbose=false&key=${OCM_API_KEY}`)
      .then(r => r.json())
  ...
  const chargersWithCost = chargers.map(c => ({ ...c, _cost: parseUsageCost(c.UsageCost) }));
  const getActiveChargers = () => {
    ...
    if (tab === "expensive") return [...chargersWithCost].sort((a,b) => b._cost - a._cost).slice(0,10);
    if (tab === "brands")    return chargersWithCost.filter(c => preferredBrands.some(b => (c.OperatorInfo?.Title || "").toUpperCase().includes(b.toUpperCase()))).sort((a,b) => a._cost - b._cost).slice(0,10);
  ...
  return (
    <>
      ...
      <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "'Inter', sans-serif", maxWidth: 480, margin: "0 auto", paddingBottom: 32 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            {updatedAt && (
              <div style={{ fontSize: 10, color: "rgba(255,255,255,0.7)", fontWeight: 500, marginTop: 2 }}>
                Última actualización MINETUR: {updatedAt}
              </div>
            )}
          </div>
          <button
            ...
            style={{
              ...
              color: "#fff", borderRadius: 10, width: 38, height: 38,
        ...
        <div style={{ padding: "16px 14px" }}>
          ...
          <div style={{ marginBottom: 14 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <span style={{ fontSize: 12, color: C.blue, fontWeight: 700, minWidth: 36, textAlign: "right" }}>{radius} km</span>
            </div>
              ...
              {fuelKey === "electric" ? (
                <div>
                  ...
                  {!chargersLoading && activeChargers.map((c, i) => {
                    ...
                    const usageCost = c.UsageCost || null;
                      ...
                      <div key={c.ID || i} style={{
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          {stationName && stationName !== operator && (
                            <div style={{ fontSize: 12, color: C.textMid, marginTop: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                              {stationName}
                            </div>
                          )}
                          <div style={{ fontSize: 12, color: C.textMuted, marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                          ...
                          <div style={{ display: "flex", gap: 6, marginTop: 5, flexWrap: "wrap" }}>
                            {connType && <span style={{ background: C.surfaceAlt, color: C.textMid, fontSize: 10, fontWeight: 600, borderRadius: 20, padding: "2px 8px" }}>{connType}</span>}
                            {power && <span style={{ background: "#1a3a2a", color: "#22c55e", fontSize: 10, fontWeight: 700, borderRadius: 20, padding: "2px 8px" }}>{power}</span>}
                          </div>
                        </div>
                        <div style={{ textAlign: "right", flexShrink: 0, alignSelf: "flex-start" }}>
                          {usageCost ? (
                            <div style={{ fontWeight: 800, fontSize: 13, color: C.text, marginBottom: 5, maxWidth: 90, wordBreak: "break-word", lineHeight: 1.3 }}>
                              {usageCost}
                            </div>
                          ) : (
                            <span style={{ color: C.textMuted, fontSize: 13, display: "block", marginBottom: 5 }}>N/D</span>
                          )}
